" vim: set fenc=utf-8 ff=unix sts=4 sw=4 ts=4 :
"
...
let g:apc_no_select = get(g:, 'apc_no_select', 0)  " not auto select, for vim before patch-7.4.775
let g:apc_omni_key = get(g:, 'apc_omni_key', ['.', '->'])  " keyword for use omni
let g:apc_path = get(g:, 'apc_path', 1)  " auto complete for path
